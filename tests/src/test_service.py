...
from func.src.domain.exceptions import EmailAlredyExists
...
async def test_when_email_alredy_exists_then_raises(mock_find_one, user_service):
    with pytest.raises(EmailAlredyExists):
        await user_service.verify_email_alredy_exists()
...
async def test_when_email_not_in_use_then_return_none_and_continue_execution(mock_find_one, user_service):
    email_in_use = await user_service.verify_email_alredy_exists()
...
async def test_when_user_not_exist_then_mocks_was_called(mock_audit, mock_social, mock_insert_one, mock_user, user_service):
    ...
    mock_audit.assert_called_once_with(user=stub_user_model)
    mock_social.assert_called_once_with(user=stub_user_model)
    mock_insert_one.assert_called_once_with(user=stub_user_model)
